...
<div class="flex items-center justify-between">
    <div>
        <h1 class="text-3xl font-bold text-white">Edit Kursus</h1>
        <p class="text-blue-100 mt-1">Perbarui informasi kursus {{ $kursus->nama }}</p>
    </div>
...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <div class="px-6 py-4 bg-gray-50 border-b border-gray-200">
            <h3 class="text-lg font-medium text-gray-900">Edit Informasi Kursus</h3>
            <p class="text-sm text-gray-600 mt-1">Perbarui form berikut untuk mengubah informasi kursus</p>
        </div>
        
        <form action="{{ route('admin.kursus.update', $kursus) }}" method="POST" enctype="multipart/form-data" class="p-6">
            @csrf
            @method('PUT')
...
>>>>>>> fixed
                ...
                <!-- Metode -->
                <div>
                    <label for="metode" class="block text-sm font-medium text-gray-700 mb-2">Metode Pembelajaran *</label>
                    <select name="metode" id="metode" 
                        ...
                        <option value="online" {{ old('metode', $kursus->metode) == 'online' ? 'selected' : '' }}>Online</option>
                        <option value="offline" {{ old('metode', $kursus->metode) == 'offline' ? 'selected' : '' }}>Offline</option>
                        <option value="hybrid" {{ old('metode', $kursus->metode) == 'hybrid' ? 'selected' : '' }}>Hybrid</option>
                    </select>
                    @error('metode')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="text" name="lokasi" id="lokasi" value="{{ old('lokasi', $kursus->lokasi) }}" 
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <input type="number" name="harga" id="harga" value="{{ old('harga', $kursus->harga) }}" 
                ...
                <div>
                    ...
                    <input type="date" name="tanggal_mulai" id="tanggal_mulai" value="{{ old('tanggal_mulai', $kursus->tanggal_mulai) }}" 
                ...
                <div>
                    ...
                    <input type="date" name="tanggal_selesai" id="tanggal_selesai" value="{{ old('tanggal_selesai', $kursus->tanggal_selesai) }}" 
                ...
                <!-- Jumlah Siswa -->
                <div>
                    <label for="jumlah_siswa" class="block text-sm font-medium text-gray-700 mb-2">Jumlah Siswa Maksimal *</label>
                    <input type="number" name="jumlah_siswa" id="jumlah_siswa" value="{{ old('jumlah_siswa', $kursus->jumlah_siswa) }}" 
                           ...
                           placeholder="Maksimal siswa" min="{{ $kursus->siswa_terdaftar }}">
                    <p class="mt-1 text-xs text-gray-500">Minimal {{ $kursus->siswa_terdaftar }} siswa (siswa yang sudah terdaftar)</p>
                    @error('jumlah_siswa')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <select name="sertifikat" id="sertifikat" 
                        ...
                        <option value="1" {{ old('sertifikat', $kursus->sertifikat) == '1' ? 'selected' : '' }}>Ya, Dengan Sertifikat</option>
                        <option value="0" {{ old('sertifikat', $kursus->sertifikat) == '0' ? 'selected' : '' }}>Tidak Ada Sertifikat</option>
                    </select>
                    @error('sertifikat')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <!-- Link WhatsApp -->
                <div class="md:col-span-2">
                    <label for="grupwa_link" class="block text-sm font-medium text-gray-700 mb-2">Link grup WhatsApp
                        *</label>
                    <input type="text" name="grupwa_link" id="grupwa_link" value="{{ old('grupwa_link', $kursus->grupwa_link) }}"
                        class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 @error('jumlah_siswa') border-red-500 @enderror"
                        placeholder="Link WhatsApp" min="1">
                    @error('grupwa_link')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <!-- Deskripsi -->
                <div class="md:col-span-2">
                    <label for="deskripsi" class="block text-sm font-medium text-gray-700 mb-2">Deskripsi Kursus *</label>
                    <textarea name="deskripsi" id="deskripsi" rows="4" 
                              ...
                              placeholder="Masukkan deskripsi lengkap kursus...">{{ old('deskripsi', $kursus->deskripsi) }}</textarea>
                    @error('deskripsi')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="flex items-center justify-end space-x-4 mt-8 pt-6 border-t border-gray-200">
                <a href="{{ route('admin.kursus.index') }}" class="px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                    Batal
                </a>
...
<script>
    ...
    function toggleLokasiField() {
        if (metodeSelect.value.toLowerCase() === 'online') {
    ...
    metodeSelect.addEventListener('change', toggleLokasiField);

    // Panggil saat halaman pertama kali dimuat
    window.addEventListener('DOMContentLoaded', toggleLokasiField);
</script>
